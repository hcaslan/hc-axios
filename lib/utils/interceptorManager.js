...
export class InterceptorManager {
  ...
  _setupConditionalInterceptor(interceptorName, condition, config) {
    ...
    this._applyConditionalInterceptor(interceptorName, { condition, config });
  }
  ...
  _applyUploadProgressLogic(data, type, config) {
    if (type === 'request' && data.data instanceof FormData) {
      ...
      data.onUploadProgress = (progressEvent) => {
        if (config.onProgress && progressEvent.lengthComputable) {
          ...
          const uploadInfo = {
            ...
            speed: progressEvent.loaded / ((Date.now() - data.uploadStartTime) || 1),
      ...
      if (config.onComplete) {
        const duration = Date.now() - data.config.uploadStartTime;
  ...
  /**
   * Apply rate limit interceptor logic
   * @private
   */
  _applyRateLimitLogic(data, type, config) {
    if (type === 'request') {
      ...
      while (this._rateLimitRequests.length > 0 && 
             now - this._rateLimitRequests[0] > windowMs) {
        this._rateLimitRequests.shift();
      }

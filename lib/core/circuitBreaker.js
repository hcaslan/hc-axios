...
export function attachCircuitBreaker(instance) {
  ...
  instance.withCircuitBreaker = function(options = {}) {
    ...
    instance.request = async function(config) {
      ...
      if (state === CircuitState.HALF_OPEN && halfOpenRequests >= maxHalfOpenRequests) {
        const error = new Error('Circuit breaker is HALF_OPEN - request limit reached');
      ...
      try {
        ...
        // Add circuit breaker info to error
        error.circuitBreakerState = state;
